angular.module("App",[])
    .run(function ($rootScope) {
        if(window.navigator.userAgent.match(/(iPhone|iPod|Android|ios|SymbianOS)/i))$rootScope.mdevice=true
    })
    ...
    .directive("currentLyric",function () {
        var obj = {
            controller:function ($scope,$rootScope,$element) {
                ...
                $scope.time = getTime()
                var listener = $rootScope.$on("timeNow",function (e,a) {
                    if(a == $scope.time){
                        $element.css({
                            fontWeight:"bold",
                            fontSize:"18px"
            ...
            link:function (scope,element,attr,crtl) {
                ...
                function init() {
                    ...
                    if(window.screen.availWidth < 450){
                        scope.cwidth = window.screen.availWidth-5
                    }else{
                        scope.cwidth = element.parent().width()
                    }
                    element.css({
                        position: "relative",
                        overflow: "hidden",
                ...
                function updateTop(top) {
                    ...
                    if(_self.Top == 0)_self.contentStep = 0
                    var cLeft = (element.width()-content.width())/2
                    content.css({
                ...
                scope.$on("newLyricIn",function (e,a) {
                    ...
                    updateTop(top)
                    // if(a.scope.$index <= 4 ){
                    //     console.log(a.scope.$index)
                    //     updateTop(-1)
                    //     return
                    // }
                    // if(a.scope.$index >= a.ele.siblings().length - 5){
                    //     updateTop(_self.maxTop + 1 )
                    //     return
                    // }
